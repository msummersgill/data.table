foverlaps <- function(x, y, by.x = if (!is.null(key(x))) key(x) else key(y), by.y = key(y), maxgap=0L, minoverlap=1L, type=c("any", "within", "start", "end", "equal"), mult=c("all", "first", "last"), nomatch=getOption("datatable.nomatch"), which = FALSE,  verbose=getOption("datatable.verbose")) {
  ...
  ## Proposed checks to give informative error messages when range columns contain NA Values
  if ( anyNA(x[[xintervals[1L]]]) )
    stop("NA values in 'x' table range column #1: ", xintervals[1L],". All rows with NA values in the range columns must be removed for foverlaps() to work")
  if ( anyNA(x[[xintervals[2L]]]) )
    stop("NA values in 'x' table range column #2: ", xintervals[2L],". All rows with NA values in the range columns must be removed for foverlaps() to work")
  if ( anyNA(y[[yintervals[1L]]]) )
    stop("NA values in 'y' table range column #1: ", yintervals[1L],". All rows with NA values in the range columns must be removed for foverlaps() to work")
  if ( anyNA(y[[yintervals[2L]]]) )
    stop("NA values in 'y' table range column #2: ", yintervals[2L],". All rows with NA values in the range columns must be removed for foverlaps() to work")
